...
class DroneNeuralControl:
    ...
    def NeuralControl(self):
        ...
        # ANEC
        E1 = (1.0 - self.TAU * self.rho) * self.N1 + self.TAU * self.N2 * self.rho
        self.BIAS = -self.TAU * self.N1 * self.Sa_t1
        E2 = (1.0 - self.TAU) * self.N2 + self.TAU * self.N1 * self.gamma + self.BIAS
        Sa = (1.0 - self.TAU * self.beta) * self.Sa_t1 + self.TAU * self.N1 * self.N2
        self.N1 = E1
        self.N2 = E2
        self.Sa_t1 = Sa

        if self.loop_count > 15000:
            self.Adaptive_bias = 1.0 if self.loop % 2 == 0 else 1.0
            self.loop += 1
            self.loop_count = 0
        else:
            self.loop_count += 1
        ...
        if self.count > 50:
            ...
            self.N3 = self.N2 - self.x_bar
            self.N4 = max(0.0, self.N3 - self.Adaptive_bias * self.sigma)
            self.N5 = max(0.0, -self.N3 - self.Adaptive_bias * self.sigma)
            self.count += 1
        else:
            self.count += 1
            self.N6 = math.tanh(3.0 * self.N4 - 3.0 * self.N5)
        ANEC_out = self.N6
        
        
        # Control output
        self.neuralYawOutput = ANEC_out + 0.5*ANOAC_out
