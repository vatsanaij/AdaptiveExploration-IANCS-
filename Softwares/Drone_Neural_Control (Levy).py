...
class DroneNeuralControl:
    def __init__(self):
        ...
        self.w_A11 = self.w_A22 = 4.5
        self.w_A12 = self.w_A21 = self.w_A12_temp = self.w_A21_temp = -4.0
        self.emu = 0.01
        self.emu_f = 0.0003
        self.imu = 0.015
        self.imu_f = 0.0003
    ...
    def NeuralControl(self):
        ...
        if (self.count > self.sampling_distance) :
            ...
            #// Define Levy Flight moving step length
            x_min = 5.0
            alpha = 1.5
            Step_uniform = np.random.uniform(0, 1)
            self.Step_value = x_min * np.power(1.0 - Step_uniform, -1.0 / (alpha - 1.0))
            self.sampling_distance = 50 * self.Step_value
            if (self.sampling_distance < 50.0):
                self.sampling_distance = 50.0
            if (self.sampling_distance > 1000.0):
                self.sampling_distance = 1000.0
